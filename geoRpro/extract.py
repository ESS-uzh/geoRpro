...
class DataExtractor:
    ...
    *********
    ...
    def save(self, fpath):
        json_arr = to_json(
            {"X": self.X, "y": self.y, "labels_map": self.labels_map},
    ...
    def add_class(self, label_name, value):
        ...
        logger.debug(
            f"Current labels ids: {self.label_ids} Sample numbers: {self.label_ids_count}"
        )
        logger.debug(
            f"Adding a new class: {label_name} with value {value} to the training data ..."
        )
        ...
        logger.debug(
            f"New labels ids: {self.label_ids} Sample numbers: {self.label_ids_count}"
        )
    ...
    def extract(cls, src, gdf, mask_value=9999):
        ...
        **********
        ...
        labels_map = {
            label_name: str(gdf.loc[gdf["classname"] == label_name, "id"].iloc[0])
        ...
        for index, geom in enumerate(geoms):
            ...
            try:
                ...
                print(
                    f"Cannot extract point: {geom}, classname: {gdf['classname'].iloc[index]} . Try the next one .."
                )
                continue
