...
def get_Xy(imgdir, gdf):
    ...
    with ExitStack() as stack_files:
        ...
        with ExitStack() as stack_action:
            ...
            X = np.array([]).reshape(0,gr_collect.metadata_collect['count']) # values
            y = np.array([], dtype=np.int64) # ids
            for w in gr_collect.gen_windows(1000):
                print(w)
                geom = box(*windows.bounds(w, gr_collect.metadata_collect['transform']))
                gdf_clipped = gpd.clip(gdf, geom)
                if not gdf_clipped.empty:
                    print('Try data extraction...')
                    r, m = gr_collect.get_window(w, mask.arr.mask, 0)
                    src = stack_action.enter_context(to_src(r, m))
                    data = DataExtractor.extract(src, gdf_clipped, mask_value=0)
                    if not data.X.size == 0:
                        print('Append data to X and y...')
                    #    y = np.append(data.y)
                    #    X = np.vstack(data.X)
    return X,y
