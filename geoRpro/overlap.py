...
def array_correction(model, target):
    """
    ...
    *************
    ...
    if neg_row_idx.size != 0:
        ...
        for r, c in zip(neg_row_idx, neg_col_idx):
            target_corr[0,r,c] = target[0,r,c]

    return target_corr.astype('uint16')
    ...
    *************
    ...
    for idx, arrays in enumerate(gen_bands(src_ref, src_target), start=1):
        ...
        if round(np.mean(diff), 5) > threshold:
            array_1_corr = array_correction(model, arrays[1])
...
def apply_band_correction(src_target, model, band=1):
    """
    ...
    *************
    ...
    for idx, w in enumerate(rst.gen_windows(src_target), start=1):
        patch_target, _ = rst.load_window(src_target, window=w, bands=[band])
        patch_target_corrected = array_correction(model, patch_target)
...
def apply_correlation_correction(src_target, corrections, outdir):
    """
    ...
    *************
    ...
    with ExitStack() as stack_action:
        rstack = rst.Rstack()
        for index in range(1, src_target.count+1):
            ...
            for band, model in corrections.items():
                if index == band:
                    ...
                    array_target = apply_band_correction(src_target, model, band)
                    break
        ...
        fname = ''.join([ntpath.basename(src_target.files[0]).split('.')[0], 'Corr']) + '.tif'
...
if __name__ == "__main__":
    ...
    for poly_ref_fname, poly_target_fname in zip(polys_refs_fname, polys_targets_fname):
        ...
        if target_fname == 'T20MPB_20200729.tif':
            apply_correlation_correction(target, band_corrections, stacks_dir)
